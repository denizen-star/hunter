# Hunter UI Redesign - LianLian Global Style Implementation Guide

## Overview
This document provides comprehensive specifications for redesigning Hunter's UI to match the clean, minimal aesthetic of LianLian Global. The design emphasizes simplicity, clarity, and professional appearance with a focus on white space and clean typography.
...
### Implementation
Replace existing color variables in `static/css/colors.css` with the new palette above.
...
**Location**: `static/css/navigation.css` or inline in templates
...
### Phase 1: Core Styles
- [ ] Update `static/css/colors.css` with new color palette
- [ ] Update `static/css/layout.css` with new layout system
- [ ] Update `static/css/navigation.css` with new sidebar styles
...
- [ ] Update `reports.html`
- [ ] Update `landing.html` (if still used)
- [ ] Update `ui.html` (new application page)
- [ ] Update `templates.html`
- [ ] Update `daily_activities.html`
...
2. **Update base template** - Ensure base.html uses new styles
3. **Update one page at a time** - Start with simplest page (landing or reports)
...
### Reference Files
- Sample rendering: `app/templates/web/sample_redesign.html`
- Application detail sample: `app/templates/web/sample_application_detail.html`
- Current styles: `static/css/*.css`
- Template files: `app/templates/web/*.html`
...
### 4. Poppins Font
Using Poppins font family (same as LianLian Global) via Google Fonts.
